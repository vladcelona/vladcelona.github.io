...
function addProductToPage(product) {
  ...
  console.log(localStorage.getItem(product['id']) === '0');
  var productList = document.querySelector('.product-list-container');
  ...
  itemImage.addEventListener('click', () => { openModal(product); });
  itemImage.src = product.image;

  var itemName = document.createElement('p');
  itemName.setAttribute('class', 'item-title');
  itemName.textContent = product.name;

  var itemPrice = document.createElement('p');
  itemPrice.setAttribute('class', 'item-price');
  itemPrice.textContent = `${formatPrice(product.price)} ${product.currencyLabel}`;

  if (localStorage.getItem(product['id']) === '0') {
    var itemButton = document.createElement('button');
    itemButton.setAttribute('class', 'item-button');
    itemButton.addEventListener('click', () => { handleAddToCart(product); });
...
<div>Добавить</div>
    ...
    itemButtonRemove.addEventListener('click', () => { handleRemoveFromCart(product); });
...
<div>
  <svg style="width: 24px; height: 24px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#FFFFFF" version="1.1" id="Capa_1" viewBox="0 0 330 330" xml:space="preserve">
    ...
    itemCount.textContent = localStorage.getItem(product['id']);

    var itemButtonAdd = document.createElement('button');
    itemButtonAdd.setAttribute('class', 'item-button-add');
    itemButtonAdd.addEventListener('click', () => { handleAddToCart(product); });
...
<div class="property-value">
  ${property.value}
    ...
    contentHeader.setAttribute('class', 'modal-content-header');
    contentHeader.setAttribute('style', 'font-weight: bold; margin-top: 16px');
    contentHeader.textContent = `Содержание`;

    const content = document.createElement('div');
    content.setAttribute('style', 'white-space: pre-line; margin-top: 16px');
...
document.addEventListener("DOMContentLoaded", function() {
  products.forEach((product) => { localStorage.setItem(product['id'], (localStorage.getItem(product['id']) === 'null') ? '0' : localStorage.getItem(product['id'])); });
  console.log(localStorage);
